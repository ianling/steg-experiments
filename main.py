...
def encode(data: bytes, resolution: tuple[int, int] = (1280, 720), tile_width: int = None, tile_height: int = None):
    """
    ...
    for byte in data:
        if frame.write(byte) == 0:
            frame.image.save(f'test_{frame_num:03d}.png')
            frame.image.close()
            frame_num += 1
...
def _smoke_test():
    ...
    encode(data_to_encode, tile_width=160, tile_height=160)

    frame_to_decode = Frame.load_from_file('test_001.png')
    assert frame_to_decode.decode() == data_to_encode[:-1]
    assert frame_to_decode.body_length == 19
    frame_to_decode = Frame.load_from_file('test_002.png')
    assert frame_to_decode.decode() == data_to_encode[-1:]
    assert frame_to_decode.body_length == 1

    images_to_video('./test_%03d.png', 'test.mp4', framerate=20)
    video_to_images('test.mp4', 'test_videoout%03d.png')

    frame_to_decode = Frame.load_from_file('test_videoout001.png')
    assert frame_to_decode.decode() == data_to_encode[:-1]
    assert frame_to_decode.body_length == 19
    frame_to_decode = Frame.load_from_file('test_videoout002.png')
    assert frame_to_decode.decode() == data_to_encode[-1:]
    assert frame_to_decode.body_length == 1


def _real_test_encode():
    with open('test_4MB.jpg', 'rb') as f:
        data = f.read()
